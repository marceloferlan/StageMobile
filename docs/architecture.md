...
3.  **Responsividade Mobile:** Todos os elementos de UI devem considerar o estado `isTablet` para adaptar tamanhos e densidade de informação. A detecção usa `smallestScreenWidthDp >= 600` (padrão Android sw600dp) — **não** `screenWidthDp`, que causa falsos positivos em phones landscape (ex: Galaxy S24 Ultra em landscape tem screenWidthDp ≈ 890dp mas smallestScreenWidthDp ≈ 452dp → phone). A LoginScreen usa layout horizontal (Row) em phone landscape (logo esquerda, form direita) para caber na altura limitada (~400dp).
